...
$(document).ready(function () {
  ...
  async function sendRequest() {
    ...
    try {
      ...
      if (data.numeros_tarea > 0) {
        for (let i = 0; i < data.numeros_tarea; i++) {
          await flakaSend(i, data.id_tareas[i]);
        }

      } else {
        console.log('No hay tareas o evidencias');
      }
    } catch (error) {
  ...
  async function flakaSend(valx, id_tareas) {
    var csrfToken = $('[name=csrfmiddlewaretoken]').val();
    ...
    try {
      const response = await $.ajax({
        ...
        url: "/probar",
        type: 'POST',
        data: { request: valx, id_tareas: id_tareas },
        dataType: 'json',
        beforeSend: function(xhr) {
          xhr.setRequestHeader('X-CSRFToken', csrfToken);
        }
      ...
      if (response) {
        // Crear un elemento de párrafo <p> para mostrar el resultado
        const resultadoElement = document.createElement('p');
        resultadoElement.textContent = response.data_evidence[0].id_tarea; // Puedes ajustar esto según la estructura de tu response

        // Agregar el elemento al div de resultados
